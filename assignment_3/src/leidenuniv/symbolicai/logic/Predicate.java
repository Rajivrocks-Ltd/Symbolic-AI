...
public class Predicate {
	...
	public boolean add;//means the predicate is an addition action in the KB
	public boolean del;//means the predicate is a deletion action in the KB
	public boolean act;//means this predicate is a world action as well
	public boolean adopt;//means this predicate is a goal addition
	public boolean drop;//means this predicate is a goal deletion
	...
	public boolean not() {
		//Special predicate to check for X!=Y we use !(X,Y) prefix notation
		//Returns true if term1!=term2
		return not&&!getTerm(0).var&&!getTerm(1).var&&!getTerm(0).term.equals(getTerm(1).term);
	}
	public boolean eql() {
		//Special predicate to check for X=Y  we use =(X,Y) prefix notation
		//Returns true if term1==term2
		return eql&&!getTerm(0).var&&!getTerm(1).var&&getTerm(0).term.equals(getTerm(1).term);
	}
	...
	public boolean isAction() {
		//returns true if this predicate is an action predicate
		//That is, either a real action (_) that is supposed to be added to the intention list,
		//or a mental action (+/-/*/~) that adds or deletes something from a kb.
	...
	public String toString() {
		...
		//So, new Predicate(pred.toString()).toString==pred.toString()
